<template>
    ...
    <v-window-item value="two">
        <v-form @submit.prevent="authStore.handleRegisterBrand(form)">
            <v-container>
                <v-text class="registro-welcome" :style="{ color: '#000000' }"> Olá, <span class="text-blue">Marca</span>
                ...
                <v-row>
                    ...
                    <!-- Form Ramo Marca -->
                    <v-col class="registro-inputs" cols="12" sm="6">
                        <v-select :ref="Categoria" v-model="form.line_of_business" :rules="categoriaRules" :items="lineStore.lineofbusiness"
                            item-title="line_of_business" item-value="line_of_business" label="Ramo" placeholder="Select..."
                            required variant="solo" prepend-inner-icon="mdi-clipboard-edit-outline">
                        </v-select>
                    </v-col>
...
const lineStore = useLineBusiness();

onMounted(() => {
    lineStore.getLineOfBusiness();
})
