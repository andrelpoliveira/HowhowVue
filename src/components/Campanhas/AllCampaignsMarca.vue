                    ...
                    <v-col v-for="card in cards" :key="card.title" :cols="card.flex">
                        <v-card :loading="loading" class="campanhas-cards mx-auto my-12">
                            ...
                            <v-row justify="center" class="campanhas-cards-periodo">
                                <v-chip-group>
                                    <v-chip class="ma-2" color="blue-lighten-4"  prepend-icon="mdi-calendar-range"
                                        :text="card.datein">
                                    </v-chip>
                                    <v-spacer></v-spacer>
                                    <v-chip class="ma-2" color="blue-lighten-4"  prepend-icon="mdi-calendar-range"
                                ...
                                <v-row justify="center" class="ma-0">
                                    ...
                                    <v-col cols="12" md="6">
                                        <v-btn @click="$emit('openEdit', {
                                            dados:{ id: card.campanhaId,
                                                    title: card.title, 
                                                    src: card.src,
                                                    text: card.text,
                                                    datein: card.datein,
                                                    dateend: card.dateend}
                                        }); idEvent = card.campanhaId;" append-icon="mdi-eye-settings" block size="x-large"
                                            color="blue-darken-1" variant="elevated" :value="card.campanhaId">Visualizar</v-btn>
                                    </v-col>
                                    <!-- <v-col cols="12" md="6">
